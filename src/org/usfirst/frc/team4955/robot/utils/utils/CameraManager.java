...
public class CameraManager {
	...
	public void InitCam(String camName){
		/*try{
			int camId = NIVision.IMAQdxOpenCamera(camName, NIVision.IMAQdxCameraControlMode.CameraControlModeController);
			cameras.put(camName, camId);
	        
		}catch (VisionException ve){
			printError(camName, "InitCam", ve);	
		}*/
	}
	
	/*private void printError(String camName, String calledMethod, VisionException ve) {
		String message = "ERROR AT : CameraManager."+calledMethod+"(" + camName + ")";
		
		if(ve.getMessage().contains("1074360310")){
			message += "\tCamera is already in use. (Called init twice with the same cam?)";
		}else if(ve.getMessage().contains("1074360311")){
	 ...
	 * */
	public void init(){
		/*for (int camId : cameras.values()) {
			try{
				//NIVision.IMAQdxStartAcquisition(camId);	
			}catch(VisionException ve){
	...
	public void SwitchCamera(String cameraName){
		...
		//Stop Current camera
		/*if(currentCamId != -1)
				NIVision.IMAQdxStopAcquisition(currentCamId);
		
		//Star the new Cam
		currentCamId = cameras.get(cameraName);
	...
	public Image GetCurrentFrame(){
		/*NIVision.IMAQdxGrab(currentCamId, currentFrame, 0);
		return currentFrame;*/
		return null;
	}
	
	public void periodic(){
		if(currentCamId == -1 ) return;
	...
	public void SetCamImage(String cameraName, Image camImage){
		if( !cameras.containsKey(cameraName) ) return;
